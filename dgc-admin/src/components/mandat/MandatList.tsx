...
export const MandatList = ({
  ...
  return (
    <>
      <ResourceContextProvider value="mandats">
        <List
          title={title}
          // filterDefaultValues={{ etat: "Emission", titre: "T1", montant: ">1M" }}
          // filters={mandatFilters}
          filter={{ etat: etat, mntcat: montant }}
          // filterDefaultValues={{ etat: "Admission", titre: "1", montant: ">1M" }}
          filters={mandatFilters}
          exporter={false}
        >
          <Datagrid optimized bulkActionButtons={false}>
            <TextField label="Titre" source="cs_titre" />
            <TextField label="Ordonnateur" source="code_ord" />
            <CountField label="N° Mandat" source="code_mandat" />
            <MoneyField label="Montant Brut" source="mt_brut" />
            <MoneyField label="Montant Net" source="mt_net" />
            {/* <DateField label="Date Reference" source="dt_ref" />
          <DateField label="Date Emission" source="dt_emission" /> */}
            {/* <DateField label="Date Disponibilite" source="dt_dispo" />
          <DateField label="Date Reglement" source="dt_reglement" /> */}
            <MonthField label="Mois" source="mois" />
            <CountField label="Gestion" source="gestion" />
